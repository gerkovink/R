...
If you are not sure which pane is the console window, use the figure below to identify the console:
![The `R` console window in `RStudio`](install.png)
---

# What if the above does not work for me?
If all fails and you have insufficient rights to your machine, the following web-based service will offer a solution. Naturally, you will need internet access for those services to be accessed. Wireless internet access will be available at the course location. 

1. You will receive an account to Utrecht University's [MyWorkPlace](https://myworkplace.uu.nl/). You would have access to `R` and `RStudio` there. You may need to install packages for new sessions. 
