...
- How to use `RStudio`, `R`-scripts and `R`-notebooks
...
- Use any, but be consistent

```{r, eval = FALSE}
student_cleaned <- 3 #studentCleaned
```

![http://journal.r-project.org/archive/2012-2/RJournal_2012-2_Baaaath.pdf](figures/naming_convention.png){width=70%}
...
CAREFUL! You always need the comma (,) when filtering by rows
...
- It'll be useful for data modeling and data isualization
```{r,collapse=TRUE}
