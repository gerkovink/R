...
First, create a new notebook for this homework (File -> New File -> R Notebook)


## Exercise 1-4: Vectors and matrixes
...
7. **Imagine that the first variable `V1` in our dataframe `dat3` is not coded correctly, but actually represents grouping information about cities. Convert the variable to a factor and add the labels Utrecht (corresponding to value 1), New York (value 2), London (3), Singapore (4), Rome (5) and Cape Town (6).** 
```{r}
...
First create a folder "data" in your project. Then download it from [here](https://javier.science/R/#day-1-nov-12th) and add it to that folder (use Ctrl+S in the browser to download it).
...
boys2.1 <- dplyr::filter(boys, age >= 20)
nrow(boys2.1)
```
...
or, alternatively, 
```{r}
boys3.2 <- dplyr::filter(boys, age > 19, age < 19.5)
nrow(boys3.2)
```
...
Download the file `example_csv.csv` from [here](http://javier.science/R/#day-1-nov-12th) and add it to your data folder.
